...
void ORC_PotatoCPPSinglePass::render_impl() {
	...
	ORC_RDHelper::get_rd()->draw_list_bind_render_pipeline(draw_list, potato_pso->pipeline);
	...
	for (int i = 0; i < potato_renderer->surfaces_data.size(); i++) {
		...
		RID albedo_uniform_set = ORC_RDHelper::get_rd()->uniform_set_create(Array::make(albedo_uniform), potato_pso->shader_program, 1);
		...
		ORC_RDHelper::get_rd()->draw_list_draw(draw_list, true, 1, 0);
